...
- Node.js ≥ 20 (backend, ESM) + React 19 / Vite 8 (frontend, existing). + Backend: Node built-ins (`node:fs`, `node:path`) + existing `pathGuard`, (003-workspace-file-viewer)
- Reads the run's git worktree on disk (live runs via the run manager's `workspacePath`; (003-workspace-file-viewer)
...
## Recent Changes
- 003-workspace-file-viewer: Added Node.js ≥ 20 (backend, ESM) + React 19 / Vite 8 (frontend, existing). + Backend: Node built-ins (`node:fs`, `node:path`) + existing `pathGuard`,
